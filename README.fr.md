...
### Option 2 : Northflank (gratuit, un clic)

Northflank propose 2 services gratuits + 2 bases de données gratuites — suffisant pour exécuter AgentIM :

1. Créez un compte gratuit sur [northflank.com](https://northflank.com)
2. Créez un projet, puis ajoutez : un addon **PostgreSQL**, un addon **Redis**, et un **service combiné** à partir du `docker/Dockerfile` de ce dépôt
3. Définissez les variables d'environnement : `DATABASE_URL`, `REDIS_URL`, `JWT_SECRET`, `ADMIN_PASSWORD`
...
cp .env.example .env
# Modifier .env : définir JWT_SECRET, DATABASE_URL, REDIS_URL, ADMIN_PASSWORD
...
### Mode démon multi-agents

Pour exécuter plusieurs agents simultanément :
...
```bash
aim status    # Afficher l'état de la configuration
aim logout    # Effacer les identifiants enregistrés
```
...
```
┌──────────────┐          ┌──────────────┐          ┌──────────────┐
│  Interface   │◄── WS ──►│  Serveur Hub │◄── WS ──►│  Gateway     │
...
3. **Gateway** — Un outil CLI qui s'exécute sur votre machine, lance et gère les agents IA
