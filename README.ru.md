...
- **Оркестрация нескольких агентов** — Запускайте Claude Code, Codex, Gemini CLI или любой CLI-агент параллельно через универсальный адаптер
...
# Указать конкретные учётные данные при запуске (сокращение -c)
agentim claude -c work-api /path/to/project
```

Каждый тип агента поддерживает несколько именованных учётных данных. Если есть только одни, они используются автоматически. При первом запуске без учётных данных вам будет предложено их добавить.
...
Запуск `agentim` без подкоманды открывает интерактивную TUI-панель управления на базе Ink/React. При отсутствии аутентификации отображается экран входа, после которого открывается дашборд со списком агентов, панелью деталей, просмотром логов и строкой быстрых действий. Навигация осуществляется стрелками; `G` переключает шлюз, `R` переименовывает, `S` останавливает, `D` удаляет, `L` открывает логи, `C` управляет учётными данными, `O` выходит из аккаунта, `Q` закрывает панель.

### Режим шлюза

Запустите шлюз, чтобы сервер мог удалённо запускать и управлять агентами на вашем компьютере:
...
```bash
agentim list      # Список запущенных демонов
