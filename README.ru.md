...
- **Оркестрация нескольких агентов** — Запускайте Claude Code, Codex, Gemini CLI *(скоро)* или любой CLI-агент параллельно через универсальный адаптер
- **Сервисные агенты** — Настройте серверные ИИ-агенты (совместимые с OpenAI), которые отвечают на @упоминания без шлюза
...
- **Управление задачами** — Создавайте, назначайте, отслеживайте и управляйте задачами прямо в чат-комнатах
...
- **Ответы в ветках** — Отвечайте на конкретные сообщения и просматривайте ветки обсуждений
- **Слеш-команды** — Используйте `/help`, `/clear`, `/task` и `/status` для быстрых действий
- **Обмен файлами** — Загружайте и делитесь файлами, изображениями и документами в чате
- **Поддержка PWA** — Установите как Progressive Web App с офлайн-режимом и push-уведомлениями
- **Тёмная тема** — Полная поддержка тёмной темы во всём интерфейсе
- **Многоязычность** — 7 языков: English, 简体中文, 日本語, 한국어, Français, Deutsch, Русский
...
- [Руководство по развёртыванию](docs/DEPLOYMENT.md) — Продакшн-настройка, Nginx, резервное копирование, устранение неполадок
- [Протокол WebSocket](docs/WEBSOCKET.md) — Типы сообщений клиента, поток аутентификации, коды ошибок
- [Руководство по адаптерам](docs/ADAPTER_GUIDE.md) — Как добавить поддержку нового типа AI-агента
- [Справка по API](docs/DEPLOYMENT.md#environment-variables) — Спецификация OpenAPI доступна по адресу `/api/docs/openapi.json`
- [Руководство по участию](CONTRIBUTING.md) — Стиль кода, тесты, процесс PR
