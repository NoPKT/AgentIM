  ...
  return (
    <div className="my-2">
      <button
        ...
        className="flex items-center gap-2 text-xs text-gray-500 hover:text-gray-700 transition-colors w-full text-left group"
      >
        <svg
      ...
      {expanded && (
        <div className="mt-1.5 ml-5 pl-3 border-l-2 border-purple-200 text-sm text-gray-600 whitespace-pre-wrap leading-relaxed max-h-80 overflow-y-auto">
          {content}
    ...
    <div className="my-2">
      ...
      >
        ...
        <span className="inline-flex items-center gap-1">
          ...
          <span className="font-medium text-blue-600">{toolName}</span>
        </span>
      </button>

      {expanded && (
        <div className="mt-1.5 ml-5 pl-3 border-l-2 border-blue-200">
          <pre className="text-xs text-gray-600 bg-gray-50 rounded-md p-2 overflow-x-auto max-h-60 overflow-y-auto">
            {content}
          </pre>
        </div>
      )}
...
export function ToolResultBlock({ content }: { content: string }) {
  ...
  return (
    <div className="my-1 ml-5 pl-3 border-l-2 border-green-200">
      <div className="text-xs text-gray-600 bg-green-50 rounded-md p-2 overflow-x-auto">
        {isLong && !expanded ? (
          <>
            <pre className="whitespace-pre-wrap">{content.slice(0, 200)}...</pre>
            <button onClick={() => setExpanded(true)} className="text-green-600 hover:text-green-700 mt-1 font-medium">
              Show more
            </button>
          </>
        ) : (
...
export function ErrorBlock({ content }: { content: string }) {
  return (
    <div className="my-2 px-3 py-2 bg-red-50 border border-red-200 rounded-lg">
      ...
      <pre className="text-xs text-red-600 whitespace-pre-wrap">{content}</pre>
    </div>
  )
}
