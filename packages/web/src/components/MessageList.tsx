...
export function MessageList() {
  ...
  return (
    ...
    <div ref={parentRef} className="h-full overflow-y-auto" role="log" aria-live="polite" aria-label={t('chat.rooms')}>
      ...
      {/* 已读回执 */}
      {currentRoomId && (() => {
        const receipts = (readReceipts.get(currentRoomId) ?? [])
          .filter((r) => r.userId !== currentUser?.id)
        if (receipts.length === 0) return null
        return (
          <div className="flex items-center gap-1 px-6 py-1.5 justify-end">
            <span className="text-xs text-gray-400 dark:text-gray-500 mr-1">{t('chat.readBy')}</span>
            ...
            {receipts.length > 5 && (
              <span className="text-xs text-gray-400 dark:text-gray-500">+{receipts.length - 5}</span>
            )}
          </div>
        )
      })()}
